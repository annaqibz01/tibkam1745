...
export const ImportPengurusModal: React.FC<ImportPengurusModalProps> = ({
  ...
  // 🎯 PARSER EXCEL + SMART SYNC ENGINE 100% AKURAT (SINGLE-QUERY FETCH)
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 2. 🔍 FETCH SELURUH DATA 'PENGURUS_SANTRI' UNTUK SYNC LOKAL (SANGAT CEPAT & AKURAT)
      ...
      try {
        ...
        console.warn(
          "[Smart Sync Error] Gagal membaca list pengurus_santri. Pastikan API Rules List/View dibuka!",
          err
        );
      }

      // 3. 🔍 LOOKUP NAMA SANTRI DARI KOLEKSI 'MASTER'
      const santriMap = new Map<string, { id: string; nama: string }>();
      if (uniqueIds.length > 0) {
        await Promise.all(
          uniqueIds.map(async (idPps) => {
            const cleanId = String(idPps).trim();
            try {
              const recordMaster = await pb
                .collection("master")
                .getFirstListItem(`id_pps = "${cleanId}"`);
              if (recordMaster) {
                const nama =
                  recordMaster.nama ||
                  recordMaster.nama_santri ||
                  recordMaster.nama_lengkap ||
                  "Tanpa Nama";
                santriMap.set(cleanId, { id: recordMaster.id, nama });
              }
            } catch {
              // Santri belum ada di master
            }
          })
        );
      }

      // 4. 🧠 MATCHING SMART SYNC ACTION LOKAL
  ...
  // 🚀 EKSEKUSI SMART SYNC (CREATE / UPDATE / SKIP)
  const handleExecuteImport = async () => {
    const processableRows = parsedRows.filter((r) => r.syncAction !== "invalid");
    ...
    for (let i = 0; i < updatedRows.length; i++) {
      ...
      try {
        if (row.syncAction === "skip") {
          ...
          await pb.collection("pengurus_santri").update(row.existingId, {
            jabatan: row.jabatan,
            status_aktif: true,
          ...
          await pb.collection("pengurus_santri").create(payload);
        ...
        failCount++;
        const pbMsg = parsePocketBaseError(err) || err.message || "Gagal Simpan";
        updatedRows[i] = {
          ...row,
          status: "error",
          message: pbMsg,
    ...
    if (failCount > 0) {
      setErrorMessage(
        `Sync Selesai: ${createCount} baru, ${updateCount} diperbarui, ${skipCount} dilewati, ${failCount} gagal.`
      );
    } else {
      handleClose();
    }
  ...
  const createRowsCount = parsedRows.filter(
    (r) => r.syncAction === "create"
  ).length;
  const updateRowsCount = parsedRows.filter(
    (r) => r.syncAction === "update"
  ).length;
  const skipRowsCount = parsedRows.filter(
    (r) => r.syncAction === "skip"
  ).length;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col h-[370px] overflow-hidden justify-between space-y-3 pt-1 select-none text-sm hide-scrollbar">
        ...
        <div className="flex items-center justify-between p-2.5 rounded-2xl bg-gray-950/80 border border-gray-800 shrink-0">
          <div className="text-xs text-gray-400">
            Kolom WAJIB: <b className="text-indigo-400 font-mono">ID PPS</b> dan{" "}
            <b className="text-purple-400 font-mono">JABATAN</b>.
          </div>
          <button
            type="button"
              ...
              {isParsing ? (
                <div className="flex flex-col items-center justify-center space-y-2">
                  ...
                  <p className="text-xs font-mono text-gray-400">
                    Membaca file Excel & menganalisis data...
                  </p>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center space-y-2">
                  ...
                  <table className="w-full text-xs text-left border-collapse font-mono">
                    <thead>
                      <tr className="bg-gray-900 border-b border-gray-800 text-gray-400 text-[10px] uppercase sticky top-0 z-10 select-none">
                        <th className="px-3.5 py-2 w-10 text-center rounded-tl-xl">#</th>
                        ...
                        <th className="px-3.5 py-2 min-w-[150px] text-left rounded-tr-xl">Aksi Smart Sync</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-800/50 bg-gray-900/20">
                      {parsedRows.map((row) => (
                        ...
                        >
                          <td className="px-3.5 py-2 text-center text-gray-500 font-mono">
                            {row.rowNum}
                          </td>
                          ...
                          <td className="px-3.5 py-2 text-gray-300 font-medium">
                            {row.jabatan || <span className="text-gray-600">-</span>}
                          </td>
                          ...
                          <td className="px-3.5 py-2 whitespace-nowrap">
                            {row.status === "success" ? (
                              ...
                              <span
                                ...
                                title={row.message}
                              >
                                <UpdateIcon className="w-3 h-3 text-amber-400" />
                                Update Jabatan
                              </span>
                            ) : row.syncAction === "skip" ? (
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold text-gray-400 bg-gray-800/60 border border-gray-800">
                                <MinusCircle className="w-3 h-3 text-gray-500" />
                                Dilewati
                              </span>
                            ) : (
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                <span>
                  Proses Import ({totalProcessable} Data)
                </span>
              </>
            )}
          </button>
