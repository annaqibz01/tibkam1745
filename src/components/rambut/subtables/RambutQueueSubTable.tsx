// src/components/rambut/subtables/RambutQueueSubTable.tsx
import React, { useMemo } from "react";
import { parseNumericIdPps, type WajibSetorExpanded } from "../../../hooks/useRambut";
import { Scissors, User, MapPin, Home, CheckCircle2, ShieldAlert, Clock } from "lucide-react";
          ...
          <tr>
            <td colSpan={9} className="px-6 py-16 text-center font-sans">
              <div className="flex flex-col items-center justify-center space-y-2">
                ...
                <p className="text-xs font-semibold text-gray-300">Tidak Ada Data Antrean Wajib Setor</p>
              </div>
            </td>
          </tr>
        ) : (
          sortedItems.map((row, index) => {
            ...
            return (
              <tr key={row.id} className="group transition-colors duration-150 hover:bg-indigo-500/[0.04]">
                ...
                <td className="px-2.5 py-2 text-gray-400 whitespace-nowrap">{santriData?.tingkatan || "-"} / {santriData?.kelas || "-"}</td>
