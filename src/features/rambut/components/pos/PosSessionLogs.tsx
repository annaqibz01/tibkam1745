...
export const PosSessionLogs: React.FC<PosSessionLogsProps> = ({ logs }) => {
  ...
  const handlePrintFromLog = async (logItem: ScanSessionLog) => {
    try {
      ...
      let logRecord = null;
      try {
        logRecord = await pb.collection("riwayat_setor_rambut").getOne(logItem.id, {
          expand: "santri,petugas_eksekutor",
        });
      } catch {
        logRecord = await pb.collection("riwayat_setor_rambut").getFirstListItem(`id_pps = "${logItem.idPps}"`, {
          sort: "-created",
          expand: "santri,petugas_eksekutor",
        });
      }
  ...
  return (
    <div className="flex flex-col bg-zinc-950/50 border border-zinc-800 rounded-xl p-3 space-y-2 h-[380px]">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar space-y-1.5 pr-1">
        {logs.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center py-10 text-xs text-zinc-600 space-y-1">
          ...
          logs.map((log) => (
            <div
              key={log.id}
              className="p-2 rounded-lg bg-zinc-900/80 border border-zinc-800 flex items-center justify-between gap-2.5 text-xs hover:border-zinc-700 transition-colors animate-in fade-in slide-in-from-top-1 duration-150"
            >
              <div className="flex items-center gap-2 truncate min-w-0">
                ...
                <div className="truncate">
                  <p className="font-bold text-white truncate">{log.namaSantri}</p>
                  <p className="text-[10px] text-zinc-400 mt-0.5">
                    ID: <span className="text-indigo-300 font-bold">{log.idPps}</span> • {log.tingkatan}/{log.kelas}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <button
                  ...
                  className="p-1.5 rounded-md bg-zinc-900 hover:bg-emerald-500/20 text-zinc-400 hover:text-emerald-300 border border-zinc-800 hover:border-emerald-500/30 transition-colors disabled:opacity-50"
