...
export const PosResultCard: React.FC<PosResultCardProps> = ({ lastResult }) => {
  ...
  return (
    <div
      className={`p-3 rounded-xl border transition-all duration-200 flex flex-col justify-between text-center h-[335px] ${
      ...
      {lastResult.status === "success" ? (
        <div className="w-full space-y-2.5 animate-in zoom-in-95 duration-150 my-auto">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-start gap-3.5 px-1 text-center sm:text-left">
            <div className="relative w-24 h-32 rounded-lg bg-zinc-900 border border-emerald-500/30 flex items-center justify-center text-white shadow-lg overflow-hidden mx-auto sm:mx-0">
              {fotoUrl && !imageError ? (
                <img
                  src={fotoUrl}
                ...
                <span className="font-sans font-extrabold text-4xl leading-none tracking-wider uppercase select-none">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-left pt-2 border-t border-emerald-500/20 font-mono text-xs">
            <div className="flex items-center gap-2 bg-zinc-950/60 p-1.5 px-2.5 rounded-lg border border-emerald-500/20">
              <GraduationCap className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
              <div className="min-w-0">
                <p className="text-[8.5px] text-zinc-400 uppercase font-semibold">Kelas</p>
                <p className="font-bold text-zinc-200 text-[10.5px] truncate uppercase">
            ...
            <div className="flex items-center gap-2 bg-zinc-950/60 p-1.5 px-2.5 rounded-lg border border-emerald-500/20">
              ...
              <div className="min-w-0">
                <p className="text-[8.5px] text-zinc-400 uppercase font-semibold">Domisili</p>
                <p className="font-bold text-emerald-300 text-[10.5px] truncate">
                  {lastResult.domisili || "-"}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 bg-zinc-950/60 p-1.5 px-2.5 rounded-lg border border-emerald-500/20 sm:col-span-2">
              <Home className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
              <div className="min-w-0 flex-1">
                <p className="text-[8.5px] text-zinc-400 uppercase font-semibold">Alamat</p>
                <p className="font-semibold text-zinc-300 text-[10.5px] truncate" title={lastResult.alamat}>
                  {lastResult.alamat || "-"}
                </p>
              </div>
            </div>
          ...
          <div className="text-[9.5px] font-mono text-emerald-400/80 pt-0.5 flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
            <div className="flex items-center gap-1">
              ...
              <span>{lastResult.tanggalHijri} {lastResult.waktu ? `(${lastResult.waktu})` : ""}</span>
            </div>
            {lastResult.penerima && (
        ...
        <div className="w-full my-auto py-4 space-y-3 animate-in zoom-in-95 duration-150 flex flex-col items-center justify-center">
          ...
          <div className="space-y-2 w-full max-w-md mx-auto">
            ...
            <p className="text-xs font-mono font-bold text-white bg-rose-950/60 p-3 rounded-lg border border-rose-500/40 leading-relaxed">
              {lastResult.message}
            </p>
          </div>
        </div>
      ) : (
        <div className="my-auto py-10 space-y-2 text-zinc-500 flex flex-col items-center justify-center">
          <Scissors className="w-9 h-9 mx-auto stroke-1" />
          <p className="text-xs font-mono font-bold">Arahkan Kartu santri ke barcode scanner</p>
        </div>
      )}
    </div>
  );
