    ...
    <table className="w-full text-xs text-left border-collapse table-auto">
      <thead>
        <tr className="bg-zinc-950 border-b border-zinc-800 text-[11px] font-semibold text-zinc-400 uppercase tracking-wider select-none">
          ...
          <th className="px-2.5 py-2.5 min-w-[130px]">Tingkatan / Kelas</th>
          <th className="px-2.5 py-2.5 w-24 text-center">Kategori</th>
          <th className="px-2.5 py-2.5 w-24 text-center">Status</th>
          <th className="px-2.5 py-2.5 w-28 text-center">Aksi</th>
        </tr>
      </thead>
          ...
          Array.from({ length: 8 }).map((_, idx) => (
            <tr key={`skel-q-${idx}`} className="animate-pulse">
              {Array.from({ length: 9 }).map((_, c) => (
                <td key={c} className="px-2.5 py-2">
          ...
          sortedItems.map((row, index) => {
            ...
            return (
              <tr key={row.id} className="hover:bg-zinc-800/40 transition-colors">
                ...
                <td className="px-2.5 py-2 font-bold text-indigo-300 whitespace-nowrap">
                ...
                <td className="px-2.5 py-2 text-indigo-300 whitespace-nowrap font-sans">
                  <div className="flex items-center gap-1">
                    ...
                    <span>{santriData?.domisili || santriData?.status_domisili || "-"}</span>
                  </div>
                </td>
                <td
                  className="px-2.5 py-2 text-zinc-400 whitespace-nowrap font-sans truncate max-w-[170px]"
                ...
                <td className="px-2.5 py-2 text-zinc-400 whitespace-nowrap">
                ...
                <td className="px-2.5 py-2 text-center whitespace-nowrap">
                  {row.status_setor === "belum" ? (
                    canExecute ? (
                      <div className="flex items-center justify-center gap-1 font-sans">
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-semibold shadow-sm transition-colors"
                        >
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-zinc-900 border border-zinc-800 text-purple-300 hover:bg-purple-500/10 text-[10px] font-semibold transition-colors"
                        >
                      ...
                      <div
                        className="inline-flex items-center justify-center gap-1 px-2 py-1 rounded-lg bg-zinc-950/60 border border-zinc-800 text-zinc-500 text-[10px] font-mono cursor-not-allowed select-none"
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[10px] font-semibold transition-colors disabled:opacity-50"
