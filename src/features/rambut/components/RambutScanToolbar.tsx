...
export const RambutScanToolbar: React.FC<RambutScanToolbarProps> = ({
  ...
  return (
    <div className="space-y-3 w-full select-none">
      ...
      <div className="flex items-center gap-4 pb-3 border-b border-gray-800/80 w-full">
        
        {/* 🛡️ KIRI: CONTAINER 3 TOGGLE TAB (TIDAK MELEBAR) */}
        <div className="flex items-center h-[42px] bg-gray-900/90 border border-gray-800/90 p-1 rounded-2xl shadow-sm flex-shrink min-w-0 overflow-hidden">
          
          <button
            type="button"
            onClick={() => onTabChange("queue")}
            className={`relative h-full px-3 rounded-xl text-xs font-mono font-bold transition-all duration-200 flex items-center justify-start gap-2 min-w-0 overflow-hidden ${
        ...
        {/* 🛡️ KANAN: KELOMPOK AKSI & BADGE PERIODE */}
        {/*
          - Tab Queue   : container kanan tidak melebar (shrink-0)
          - Tab Pengurus/Audit : container kanan mengambil sisa ruang (flex-1 min-w-0)
            agar tombol/badge bisa melebar penuh
        */}
        <div
          className={`flex items-center gap-2.5 ${
            activeTab === "queue" ? "shrink-0" : "flex-1 min-w-0"
          }`}
        >
          
          {/* 🌙 BADGE INFORMASI PERIODE */}
          {/*
            - Semua tab: badge selalu shrink-0 (tidak mengecil) + whitespace-nowrap
            - Khusus tab audit: badge satu-satunya di kanan, jadi flex-1 agar membesar penuh
          */}
          <div
            className={`flex items-center gap-2 px-3.5 h-[42px] rounded-2xl bg-gray-900/80 border border-gray-800/90 shadow-sm backdrop-blur-xl text-xs font-mono shrink-0 whitespace-nowrap ${
              activeTab === "audit" ? "flex-1" : ""
            }`}
          >
          ...
          {/* TOMBOL AKSI BERDASARKAN TAB (queue / pengurus) */}
          
          {/* Tab Queue: tombol Generate & POS tetap kecil */}
          {activeTab === "queue" && (
            <>
              ...
              {onOpenPosModal && (
                ...
                >
                  <Zap className="w-3.5 h-3.5 shrink-0 text-amber-300 animate-pulse" />
          ...
          {/* Tab Pengurus: tombol Tambah Pengurus melebar penuh */}
